...
# Feature engineering
data['is_reordered'] = data['reordered'].apply(lambda x: 1 if x > 0 else 0)
...
predictions_df.to_csv('user_product_predictions.csv', index=False)
...
# Prepare features and target
features = ['order_dow', 'order_hour_of_day', 'days_since_prior_order', 'add_to_cart_order', 
            'user_order_number_max', 'user_days_since_prior_order_mean', 'product_reordered_mean', 'product_add_to_cart_order_mean']
X = data[features]
y = data['is_reordered']
...
cat_features_indices = [features.index('order_dow')]
...
# Function to predict the next product for a user
def predict_next_product2(user_id, model, data, features):
    # Get data for the specific user
    user_data = data[data['user_id'] == user_id].copy()

    # Predict probabilities
    user_data.loc[:, 'prediction'] = model.predict_proba(user_data[features])[:, 1]

    # Get the top product prediction
    top_product = user_data.sort_values('prediction', ascending=False).head(1)[['product_id', 'product_name']]
    return top_product
...
for user_id in all_users:
    top_products = predict_next_products2(user_id, model, data, features, n=5)
    for product_id in top_products:
        predictions2.append({'user_id': user_id, 'product_id': product_id})
...
# Map product IDs to product names
product_names = data.set_index('product_id')['product_name'].to_dict()
predictions_df2['product_name'] = predictions_df2['product_id'].map(product_names)

# Save predictions to CSV
predictions_df2.to_csv('user_product_predictions2.csv', index=False)

print("Predictions saved to user_product_predictions2.csv")
...
# Function to predict top N products for a user
def predict_next_products(user_id, model, data, features, n=1):
    # Get data for the specific user
    user_data = data[data['user_id'] == user_id].copy()
    ...
    top_products = user_data.sort_values('prediction', ascending=False).head(n)['product_id']
    return top_products
...
for user_id in all_users:
    top_products = predict_next_products(user_id, model, data, features, n=1)
    for product_id in top_products:
        predictions.append({'user_id': user_id, 'product_id': product_id})
...
# Save predictions to CSV
predictions_df.to_csv('user_product_predictions4.csv', index=False)

print("Predictions saved to user_product_predictions4.csv")
